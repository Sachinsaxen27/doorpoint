...
// ROUTER 4 FOR GET ALL TOYS LIST
router.get('/getlist',async(req,res)=>{
    const toyslist=await Toys.find({},{itemid:1,name:1,category:1,producttype:1,age:1,image:1,brand:1, material:1,color:1,dimensions:1,price:1,recommandplayer:1,information:1,features:1,includeaccess:1,notes:1,rechargeable:1,fieldsection:1})
    res.status(200).json(toyslist)
})
// ROUTER 5 FOR GET ALL BOOKS LIST
router.get('/getbook',async(req,res)=>{
    const booklist=await Books.find({},{itemid:1,name:1,itemtype:1,producttype:1,publishdate:1,publisher:1,binding:1,nopage:1,price:1,image:1,information:1,languages:1,edition:1,author:1,availablelanguages:1,fieldsection:1})
    res.status(200).json(booklist)
})
// ROUTER 6 FOR BOTH TOYS/BOOKS LIST
router.get('/allTBlist',async(req,res)=>{
    const booklist=await Books.find({},{itemid:1,name:1,itemtype:1,producttype:1,publishdate:1,publisher:1,binding:1,nopage:1,price:1,image:1,information:1,languages:1,edition:1,author:1,availablelanguages:1,fieldsection:1})
    const toyslist=await Toys.find({},{itemid:1,name:1,category:1,producttype:1,age:1,image:1,brand:1, material:1,color:1,dimensions:1,price:1,recommandplayer:1,information:1,features:1,includeaccess:1,notes:1,rechargeable:1,fieldsection:1})
    res.status(200).json({booklist,toyslist})
})
